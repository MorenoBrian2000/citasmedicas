...
class PDF extends FPDF
{
    ...
    function Header()
    {
      ...
      function getData()
    {
        $smtp = Conexion::conectar()->prepare("SELECT T1.*, T2.correo_paciente, T2.telefono_paciente, T2.domicilio_paciente, CONCAT_WS(' ', T2.nombre_paciente, T2.apaterno_paciente, T3.amaterno_medico) AS nombre_paciente, CONCAT_WS(' ', T3.nombre_medico, T3.apaterno_medico, T3.amaterno_medico) AS nombre_medico
        FROM tbl_citas T1
        INNER JOIN tbl_pacientes T2 ON T1.id_paciente = T2.id_paciente
        INNER JOIN tbl_medicos T3 ON T1.id_medico = T3.id_medico 
            WHERE T1.id_cita = '$_GET[folio_cita]'");
